...
class Database
{
    ...
    {
        $stmt = $this->pdo->prepare("SELECT id, name, telegram_username, birthday_chat_id, birth_date FROM birthdays WHERE user_id = ?");
        $stmt->execute([$userId]);
    ...
    {
        $today = date('m-d');
        $stmt = $this->pdo->prepare("SELECT b.name, b.telegram_username, b.birthday_chat_id, b.birth_date, u.chat_id FROM birthdays b JOIN users u ON b.user_id = u.user_id WHERE DATE_FORMAT(b.birth_date, '%m-%d') = ?");
        $stmt->execute([$today]);
        return $stmt->fetchAll(PDOConnection::FETCH_ASSOC);
    }
    ...
    {
        $stmt = $this->pdo->prepare("SELECT birthday_chat_id FROM birthdays WHERE telegram_username = ? LIMIT 1");
        $stmt->execute([$username]);
        $result = $stmt->fetch(PDOConnection::FETCH_ASSOC);
        return $result ? (int) $result['birthday_chat_id'] : null;
    }
}
